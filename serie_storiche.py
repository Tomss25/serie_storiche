...
raw_input = st.sidebar.text_area(
    ...
    value="SP500\nNASDAQ\nSWDA.MI\nLU1287022708", 
    height=150,
    help="Puoi scrivere 'SP500', 'NASDAQ', 'GOLD' o i ticker classici."
)

# --- 2. INTELLIGENZA ALIAS (NUOVA FUNZIONE) ---
# Mappa i nomi comuni ai Ticker corretti di Yahoo
ALIAS_MAP = {
    "SP500": "^GSPC",
    "S&P500": "^GSPC",
    "NASDAQ": "^NDX",
...
# Regex aggiornata: Ora accetta anche il carattere '^' e '='
raw_tokens = re.findall(r"[\w\.\-\^\=]+", raw_input.upper())

# Sostituzione automatica degli Alias
tickers_input = []
for t in raw_tokens:
    if t in ALIAS_MAP:
        ...
        with st.spinner('Ricerca in corso...'):
            for t in tickers_input:
                ...
                if series is not None:
                    # Rinonimo col nome originale se era un alias (opzionale, qui teniamo il ticker tecnico)
                    series.name = t 
                    all_series[t] = series
                else:
